...
**Design philosophy — deep modules.** Following Ousterhout's *A Philosophy of Software Design*: modules should have **narrow interfaces hiding deep implementations**. The user asked for something "very modular" *and* "deep modules" — two forces that pull in opposite directions if applied naively (atomic design tempts you toward dozens of shallow modules). We reconcile them with one rule:

> **The Nx *library* is the deep module. Standalone components are its internal units.**
...
- Angular 21's native Vitest builder is *stable but young*; some CJS/directory-import libs choke under it.
...
export type { ButtonVariant, ButtonSize } from './lib/atoms/button/button.variants';
// molecules
...
**Atomic direction inside the design system.** Nx tags act on *project* boundaries, so the one-way atomic arrow (atoms ← molecules ← organisms ← templates) is enforced *within* the lib via ESLint `no-restricted-imports`, scoped to `libs/design-system/**`:
...
Result: dependency direction is guaranteed both *between* libraries (Nx graph) and *within* the design system (ESLint).
...
export class ButtonComponent {
  ...
  protected readonly classes = computed(() => button({ intent: this.intent(), size: this.size() }));
}
```
...
export const cn = extendTailwindMerge({ /* register custom token groups if needed */ });
```
