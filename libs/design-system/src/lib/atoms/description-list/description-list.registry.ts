...
export class DescriptionPluginRegistry {
  private readonly byType = new Map<string, Type<DescriptionValueComponent>>();

  constructor() {
    const defaults = inject(DESCRIPTION_DEFAULT_VALUE_PLUGINS, { optional: true }) ?? [];
    const registered = inject(DESCRIPTION_VALUE_PLUGINS, { optional: true }) ?? [];
